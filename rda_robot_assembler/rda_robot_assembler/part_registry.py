...
■ 코드 편집 없이 모델 추가하기 (폴더 드롭)
  모델 폴더(기본 `~/robot_ws/src/rda_robot_description/config/models/<슬롯>/`,
...
def models_dir():
    """모델 드롭 폴더. 환경변수 RDA_MODELS_DIR 우선, 없으면 description/config/models."""
    d = os.environ.get("RDA_MODELS_DIR")
    if d:
        return os.path.expanduser(d)
    return os.path.expanduser(
        "~/robot_ws/src/rda_robot_description/config/models")
