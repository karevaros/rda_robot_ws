...
### 3-6-1. 수확 방식 두 가지 — 열매 파지(grasp) vs 화방대 절단(cut)

토마토는 두 가지로 딴다. 지금까지 본 것은 **열매를 하나씩 잡는(grasp)** 방식이고, 실제
현장에서는 열매에 손상이 가지 않도록 **주 줄기와 열매 사이의 화방대를 잘라(cut)** 화방을
통째로 거두는 방식도 쓴다. `harvest_mode` 로 고른다(기본 `grasp` — 종전 동작 그대로).

```bash
# 화방대 절단 모드 — 도달 가능한 화방대 목록만 뽑아 본다
ros2 launch rda_robot_moveit_config pregrasp_demo.launch.py \
    harvest_mode:=cut scan_all:=true
```

**두 모드는 기하 제약이 다르다.** 파지는 접근축이 열매를 향하기만 하면 되고 손목 롤은
자유다. 절단은 두 가지를 함께 만족해야 한다:

| | 파지(grasp) | **절단(cut)** |
|---|---|---|
| 목표 | 열매(`fruit_*`, kind:target) | **화방대(`rachis_*`, kind:obstacle)** |
| 접근축 | 열매를 향하면 됨 | **줄기 축에 수직** (비스듬하면 날이 미끄러진다) |
| 손목 롤 | 자유(기본 0) | **계산값** — 닫힘축이 줄기에 수직이어야 가로질러 잘린다 |
| 자유변수 | φ·θ·ψ 격자 | **줄기 축 둘레 각 β 하나** (`cut_beta_deg`) |
| 목표 기하 | 반경 r 의 구(ACM ρ 영역) | **점 하나** — 자를 그 객체만 이름으로 허용 |
| tcp 정지점 | 열매 앞 `grasp_offset`(13.1cm) | 절단점 앞 `cut_offset`(패드 중앙 15.2cm) |
| 그리퍼 | `gripper_close` 0.35 | `cut_gripper_close` **0.0**(끝까지) |
| 수확 결과 | 열매 1개 제거 | **화방대 + 딸린 열매 전부** 제거 |

> 🔴 **롤이 자유변수가 아닌 것이 핵심이다.** 파지용 탐색공간(`sample_psi_deg=[0.0]`)에서는
> RG2 의 닫힘축이 이 온실 화방대와 **20.9°** 밖에 안 벌어진다 — 아무리 닫아도 줄기가 패드
> 사이를 가로지르지 못해 **원리적으로 못 자른다**. 필요한 롤은 **−69.8°**(β=0 기준)이고,
> `cut` 모드는 매 β 마다 이 롤을 닫힌 형태로 풀어 넣는다(실행 로그의 `닫힘축∠줄기 90.0°`).

닫힘축은 URDF 손가락 링크에서 유도한다(`robot_introspect.gripper_closing_axis`, RG2 실측
`(1,0,0)`) — 그리퍼를 바꾸면 따라간다.

주요 인자: `cut_ratio`(절단점 위치 0=줄기쪽·1=열매쪽) · `cut_depth`(날 깊이) ·
`cut_beta_deg`(줄기 둘레 접근각) · `cut_object`+`cut_axis`(좌표 목표로 줄 때).

> ⚠ **실측 한계 — 기본 구성으로는 한 개도 못 자른다(0/21).** 원인은 도달거리가 아니라
> **거터**다. `truss.first_z` 를 0.35→0.10 으로 낮춰 둔 탓에 화방이 거터 상면에서 **8.2cm**
> 위에 있는데, 수직 절단 자세는 손가락이 세로로 서므로 그 틈에 안 들어간다(막은 접촉 =
> `rg2_*|gutter_front`). 팔 베이스를 +500mm 올린 스탠드 구성에서는 **8/21**, 화방 높이를
> 현실값 `first_z=0.35` 로 되돌리면 **5/21**(z 1.25~1.50m)이 되고 이때 막는 것은 거터가
> 아니라 **주 줄기**(`rg2_*|stem_*`)다. ⇒ 절단 방식은 **스탠드 채택 여부와 묶여 있다.**
> 실패 원인을 갈라 보려면 `screen_why_detail:=true`(접촉 쌍까지 기록).
